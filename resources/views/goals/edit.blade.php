        ...
        <div class="card-body">
            <form action="{{ route('goals.update', $goal) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="3" required>{{ old('description', $goal->description) }}</textarea>
                </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select" id="period" name="period" required>
                            @foreach ($periods as $period)
                                <option value="{{ $period }}" {{ old('period', $goal->period) == $period ? 'selected' : '' }}>
                                    {{ ucfirst($period) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select" id="status" name="status" required>
                             @foreach ($statuses as $status)
                                <option value="{{ $status->value }}" {{ old('status', $goal->status->value) == $status->value ? 'selected' : '' }}>
                                    {{ $status->value }}
                                </option>
                            @endforeach
                        </select>
